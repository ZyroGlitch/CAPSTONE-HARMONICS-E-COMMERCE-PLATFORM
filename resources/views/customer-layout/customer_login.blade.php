<nav class="navbar navbar-expand-lg bg-light shadow-sm">
    <div class="container-fluid">
        <a class="navbar-brand fw-bold text-dark" href="{{ route('customer-login.dashboard') }}"
            @class(['customer-nav-current' => request()->is('/dashboard')]) wire:navigate>
            ...
            <ul class="navbar-nav mx-auto mb-2 mb-lg-0 gap-5">
                ...
                <li class="nav-item">
                    <a class="nav-link fs-5 fw-bold @if (request()->is('orders')) customer-nav-current @endif"
                        href="{{ route('customer-login.orders') }}" @class(['customer-nav-current' => request()->is('/orders')]) wire:navigate>ORDERS
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link fs-5 fw-bold @if (request()->is('messages')) customer-nav-current @endif"
                        href="{{ route('customer-login.messages') }}" @class(['customer-nav-current' => request()->is('/messages')])
                        wire:navigate>MESSAGES</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link fs-5 fw-bold @if (request()->is('location')) customer-nav-current @endif"
                        href="{{ route('customer-login.location') }}" @class(['customer-nav-current' => request()->is('/location')])
